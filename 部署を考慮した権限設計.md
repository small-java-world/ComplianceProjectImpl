...
CREATE TABLE PermissionDetail (
    ...
    department_scope     VARCHAR(50)   NOT NULL,   -- "ANY_DEPT", "OWN_DEPT", "DEPT_XXX", "SPECIFIC" など
...
1. **`department_scope` に文字列を設定**  
   ...
   - `"OWN_DEPT"` → 自部門や下位部門のみ  
   - `"DEPT_ABC"` → 単一部門  
   - `"SPECIFIC"` → 複数部門を PermissionDetail_Department で管理するケース  
...
3. **"DEPT_XXX"**  
   - 単一の部門 → 中間テーブルは不要  
   - 例: `"DEPT_AUDIT"`, `"DEPT_SEC"` など、実際には Department.department_code や ID を格納
4. **"SPECIFIC"**  
   - 複数部門 → PermissionDetail_Department に個別行を登録
